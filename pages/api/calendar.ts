...
export default async (req: NextApiRequest, res: NextApiResponse) => {
	...
	if (token && token.accessToken) {
		try {
			...
			let eventsList: (calendar_v3.Schema$Event & { color: string })[] = [];
			for (const item of calendarList.data.items!!) {
				...
				const timeMax = `${nextMonthYear}-${nextMonth
					.toString()
					.padStart(2, '0')}-01T00:00:00-07:00`;
				...
				for (const event of events.data.items!!) {
					if (event.recurrence) {
						...
						const formattedEvents = instances.data.items!!.map(x => ({
							...x,
							color: item.backgroundColor!!,
						}));
						eventsList.push(...formattedEvents);
					} else {
						eventsList.push({ ...event, color: item.backgroundColor!! });
					}
				}
			}

			const colors = (await calendar.colors.get()).data.calendar;

			res.json({ events: eventsList, colors });
		} catch {
			res.status(500).json({ events: [], colors: {} });
		}
	}
